!function() {
    ...
    function o(e, n) {
        ...
        y++, e.as = q, e.at = m, e.rq_c = y, e.ifo = h, e.ak = d.app_key, e.uu = g, e.v = f, 
        e.st = Date.now(), e.ev = n, e.wsr = S, e.ufo = i(e.ufo), e.ec = _, wx.Queue.push(t);
    }
    function i(e) {
    ...
    "" === d.app_key && console.error("请在配置文件中填写您的app_key"), d.app_key = d.app_key.replace(/\s/g, ""), 
    function() {
        wx.request({
            url: "https://" + l + ".aldwx.com/config/app.json",
            method: "GET",
            success: function(e) {
                200 === e.statusCode && (e.data.version != f && console.warn("您的SDK不是最新版本，请尽快升级！"), 
    ...
    var h = "", g = function() {
        ...
        return e;
    }(), p = {}, v = "", w = "", _ = 0, y = "", S = wx.getLaunchOptionsSync(), x = Date.now(), m = "" + Date.now() + Math.floor(1e7 * Math.random()), q = "" + Date.now() + Math.floor(1e7 * Math.random()), k = 0, M = "", b = "", O = !0, D = [ "aldSendEvent", "aldOnShareAppMessage", "aldShareAppMessage", "aldSendSession", "aldSendOpenid" ];
    (function() {
        return Promise.all([ n(), t(), a() ]);
    })().then(function(e) {
        "" !== e[2] ? (p.lat = e[2].latitude || "", p.lng = e[2].longitude || "", p.spd = e[2].speed || "") : (p.lat = "", 
        p.lng = "", p.spd = ""), "" !== e[1] ? p.nt = e[1].networkType || "" : p.nt = "";
        var n = c(p);
        "" !== e[0] && (n.ufo = e[0], M = e[0]), o(n, "init");
    }), wx.onShow(function(e) {
        y = 0, S = e, k = Date.now(), O || (m = "" + Date.now() + Math.floor(1e7 * Math.random()), 
        h = !1, wx.setStorageSync("ald_ifo", !1)), O = !1;
        var n = c(p), t = c(p);
        n.sm = k - x, e.query.ald_share_src && e.shareTicket && "1044" === e.scene ? (t.tp = "ald_share_click", 
        r().then(function(e) {
            t.ct = e, o(t, "event");
        })) : e.query.ald_share_src && (t.tp = "ald_share_click", t.ct = "1", o(t, "event")), 
        o(n, "show");
    }), wx.onHide(function() {
        var e = c(p);
    ...
    for (var I = {
        aldSendEvent: function(e, n) {
            ...
            o(t, "event")) : "object" == typeof n ? (JSON.stringify(n).length >= 255 && console.error("自定义事件参数不能超过255个字符"), 
            t.ct = JSON.stringify(n), o(t, "event")) : void 0 === n || "" === n ? o(t, "event") : console.error("事件参数必须为String,Object类型,且参数长度不能超过255个字符")) : console.error("事件名称必须为String类型且不能超过255个字符");
        },
    ...
    try {
        ...
        p.br = T.brand || "", p.md = T.model, p.pr = T.pixelRatio, p.sw = T.screenWidth, 
        p.sh = T.screenHeight, p.ww = T.windowWidth, p.wh = T.windowHeight, p.lang = T.language, 
        p.wv = T.version, p.sv = T.system, p.wvv = T.platform, p.fs = T.fontSizeSetting, 
        p.wsdk = T.SDKVersion, p.bh = T.benchmarkLevel || "", p.bt = T.battery || "", p.wf = T.wifiSignal || "", 
